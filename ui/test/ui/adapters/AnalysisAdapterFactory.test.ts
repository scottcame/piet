...
test('table model construction', async () => {
  ...
  return adapterFactory.getTableModel(repo.analyses).then(async (tableModel: TableModel<Analysis>) => {
...
test('table model events', async () => {
  ...
  const repo: LocalRepository = new LocalRepository();
  const tableModel: TableModel<Analysis> = await adapterFactory.getTableModel(repo.analyses);
...
test('row filtering', async () => {
  ...
  const tableModel: TableModel<Analysis> = await adapterFactory.getTableModel(repo.analyses, excludedAnalyses);
  ...
  await repo.init().then(async () => {
    await repo.browseAnalyses().then(async (analyses: List<Analysis>) => {
      await excludedAnalyses.add(analyses.get(0)).then(async () => {
        await repo.browseAnalyses().then((_analyses: List<Analysis>) => {
